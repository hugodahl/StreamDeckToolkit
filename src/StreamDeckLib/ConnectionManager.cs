...
namespace StreamDeckLib
{
	...
	public partial class ConnectionManager : IDisposable
	{
		...
		private RegistrationParameters  _RegistrationParameters { get; }

		private ConnectionManager(RegistrationParameters config)
		{
			this._RegistrationParameters = config;
		}
		                                           ...
		                                           IStreamDeckProxy streamDeckProxy = null)
		{
			using (var app = new CommandLineApplication())
			{
				...
				var config = new RegistrationParameters(optionPort.ParsedValue, optionPluginUUID.Value(), optionInfo.Value(), optionRegisterEvent.Value());

				try
				{
					return Initialize(config, loggerFactory, streamDeckProxy ?? new StreamDeckProxy());
				}
				catch
				{
					throw new
		...
		private static ConnectionManager Initialize(RegistrationParameters config, 
		                                            ILoggerFactory loggerFactory,
		                                            IStreamDeckProxy       streamDeckProxy)
		{
			// TODO: Validate the info parameter
			var myInfo = JsonConvert.DeserializeObject<Messages.Info>(config.Info);
		...
		public async Task<ConnectionManager> StartAsync()
		{
			using (var source = new CancellationTokenSource())
			{
				...
				await Task.Factory.StartNew(() => Run(source.Token), TaskCreationOptions.LongRunning);
		...
		{
			await _Proxy.ConnectAsync(new Uri($"ws://localhost:{_RegistrationParameters.Port}"), token);
			await _Proxy.Register(_RegistrationParameters.RegisterEvent, _RegistrationParameters.PluginUUID);
...
#region StreamDeck Methods
		...
		public async Task SendToPropertyInspectorAsync(string context, dynamic payload)
		{
			var args = new SendToPropertyInspectorArgs
			           {
				           action  = _RegistrationParameters.PluginUUID,
				           context = context,
				           payload = payload
			           };
